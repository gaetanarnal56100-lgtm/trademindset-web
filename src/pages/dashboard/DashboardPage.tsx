...
// ── P&L Curve — interactive canvas ────────────────────────────────────────

type PnLPeriod = '1W'|'1M'|'3M'|'6M'|'ALL'
const PERIOD_MS: Record<PnLPeriod,number> = {'1W':7,'1M':30,'3M':90,'6M':180,'ALL':0}

interface PnLPoint {
  date:Date; cumPnL:number; tradePnL:number; drawdown:number; peakPnL:number
  symbol:string; direction:string
}

function buildPnLData(trades:Trade[], period:PnLPeriod):PnLPoint[] {
  const now=Date.now(), days=PERIOD_MS[period]
  const cutoff=days>0?new Date(now-days*86400000):new Date(0)
  const cl=[...trades]
    .filter(t=>t.status==='closed'&&safeTime(t.date)>=cutoff.getTime())
    .sort((a,b)=>safeTime(a.date)-safeTime(b.date))
  let cum=0,peak=0
  return cl.map(t=>{
    const p=tradePnL(t); cum+=p
    if(cum>peak)peak=cum
    const dd=peak>0?((peak-cum)/Math.abs(peak))*100:0
    return{date:new Date(safeTime(t.date)),cumPnL:Math.round(cum*100)/100,
      tradePnL:Math.round(p*100)/100,drawdown:Math.round(dd*10)/10,peakPnL:Math.round(peak*100)/100,
      symbol:t.symbol,direction:t.type}
  })
}
...
function PnLCurve({trades}:{trades:Trade[]}) {
  ...
  const data=useMemo(()=>buildPnLData(trades,period),[trades,period])
  const isPos=(data[data.length-1]?.cumPnL??0)>=0
  const lineColor=isPos?'#22C759':'#FF3B30'
  ...
  useEffect(()=>{
    ...
    const PAD={top:16,right:12,bot:32,left:64}
    ...
    for(let i=0;i<=4;i++){
      ...
      ctx.fillStyle='#3D4254';ctx.font='10px JetBrains Mono,monospace';ctx.textAlign='right'
      const abs=Math.abs(v),sign=v>=0?'+':'-'
      const label=abs>=1000?`${sign}$${(abs/1000).toFixed(1)}k`:`${sign}$${abs.toFixed(0)}`
    ...
    if(showDD){
      ...
      ctx.beginPath()
      data.forEach((d,i)=>i===0?ctx.moveTo(toX(i),toY(d.peakPnL)):ctx.lineTo(toX(i),toY(d.peakPnL)))
      for(let i=data.length-1;i>=0;i--)ctx.lineTo(toX(i),toY(data[i].cumPnL))
      ctx.closePath();ctx.fill()
    }

    // Fill under curve
    const grad=ctx.createLinearGradient(0,PAD.top,0,PAD.top+cH)
    grad.addColorStop(0,lineColor+'28');grad.addColorStop(0.7,lineColor+'05');grad.addColorStop(1,lineColor+'00')
    ctx.beginPath()
    data.forEach((d,i)=>i===0?ctx.moveTo(toX(i),toY(d.cumPnL)):ctx.lineTo(toX(i),toY(d.cumPnL)))
    ctx.lineTo(toX(data.length-1),Math.min(PAD.top+cH,Math.max(zY,PAD.top+cH)))
    ctx.lineTo(toX(0),Math.min(PAD.top+cH,Math.max(zY,PAD.top+cH)))
    ...
    data.forEach((d,i)=>{
      ...
      ctx.beginPath();ctx.arc(x,y,isHover?5:2,0,Math.PI*2)
      ctx.fillStyle=isHover?c:c+(i===hoverIdx?'ff':'88');ctx.fill()
      if(isHover){ctx.strokeStyle='#fff';ctx.lineWidth=1.5;ctx.stroke()}
    })

    // X labels
    const step=Math.ceil(data.length/Math.min(7,data.length))
    ctx.fillStyle='#3D4254';ctx.font='10px JetBrains Mono,monospace';ctx.textAlign='center'
    data.forEach((d,i)=>{
      if(i%step===0||i===data.length-1)
        ctx.fillText(d.date.toLocaleDateString('fr-FR',{day:'2-digit',month:'2-digit'}),toX(i),H-8)
    })

    // Crosshair
    if(hoverIdx!==null){
      const x=toX(hoverIdx),y=toY(data[hoverIdx].cumPnL)
      ctx.strokeStyle='rgba(255,255,255,0.2)';ctx.lineWidth=1;ctx.setLineDash([4,4])
      ctx.beginPath();ctx.moveTo(x,PAD.top);ctx.lineTo(x,PAD.top+cH);ctx.stroke()
      ctx.beginPath();ctx.moveTo(PAD.left,y);ctx.lineTo(W-PAD.right,y);ctx.stroke()
      ctx.setLineDash([])
    }
  },[data,W,H,hoverIdx,lineColor,showDD])

  const handleMouseMove=(e:React.MouseEvent<HTMLCanvasElement>)=>{
    if(data.length<2)return
    const rect=canvasRef.current!.getBoundingClientRect()
    const PAD_LEFT=64,PAD_RIGHT=12
    const pct=Math.max(0,Math.min(1,(e.clientX-rect.left-PAD_LEFT)/(W-PAD_LEFT-PAD_RIGHT)))
    ...
    <div>
      ...
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:12,flexWrap:'wrap',gap:8}}>
        <div style={{display:'flex',background:'#0D1117',borderRadius:7,padding:2,gap:1,border:'1px solid #1E2330'}}>
          {(['1W','1M','3M','6M','ALL'] as PnLPeriod[]).map(p=>(
            <button key={p} onClick={()=>setPeriod(p)} style={{padding:'3px 9px',borderRadius:5,fontSize:10,fontWeight:600,cursor:'pointer',border:'none',fontFamily:'JetBrains Mono,monospace',background:period===p?'#1C2130':'transparent',color:period===p?'#F0F3FF':'#555C70',outline:period===p?'1px solid #2A2F3E':'none'}}>{p}</button>
          ))}
        </div>
        <button onClick={()=>setShowDD(x=>!x)} style={{display:'flex',alignItems:'center',gap:5,padding:'3px 9px',borderRadius:6,fontSize:10,fontWeight:500,cursor:'pointer',border:`1px solid ${showDD?'rgba(255,59,48,0.35)':'#2A2F3E'}`,background:showDD?'rgba(255,59,48,0.08)':'transparent',color:showDD?'#FF3B30':'#555C70'}}>
          <div style={{width:7,height:7,borderRadius:1,background:showDD?'#FF3B30':'#555C70'}}/>Drawdown
        </button>
      </div>

      {/* Canvas */}
      {data.length<2?(
        <div style={{height:220,display:'flex',alignItems:'center',justifyContent:'center',color:'#3D4254',fontSize:13}}>Pas encore assez de trades fermés</div>
      ):(
        <div ref={containerRef} style={{position:'relative'}}>
          <canvas ref={canvasRef} onMouseMove={handleMouseMove} onMouseLeave={()=>setHoverIdx(null)}
            style={{width:'100%',height:H,display:'block',cursor:'crosshair'}} width={W} height={H}/>

          {/* Tooltip */}
          {hovered&&(()=>{
            const rect=containerRef.current?.getBoundingClientRect()
            const PAD_LEFT=64,PAD_RIGHT=12,cW=W-PAD_LEFT-PAD_RIGHT
            const dotX=PAD_LEFT+(hoverIdx!/(data.length-1))*cW
            const tW=190,left=Math.min(dotX+12,W-tW-8),top=8
            return(
              <div style={{position:'absolute',left,top,width:tW,background:'#161B22',border:'1px solid #2A2F3E',borderRadius:10,padding:'10px 12px',pointerEvents:'none',boxShadow:'0 8px 24px rgba(0,0,0,0.5)'}}>
                <div style={{display:'flex',justifyContent:'space-between',marginBottom:7,fontSize:10,color:'#555C70'}}>
                  <span>{hovered.date.toLocaleDateString('fr-FR',{day:'2-digit',month:'short',year:'2-digit'})}</span>
                  <span style={{color:hovered.tradePnL>=0?'#22C759':'#FF3B30',fontWeight:700}}>{hovered.direction==='Long'?'▲':'▼'} {hovered.symbol}</span>
                </div>
                <div style={{display:'flex',justifyContent:'space-between',marginBottom:5}}>
                  <span style={{fontSize:10,color:'#555C70'}}>Ce trade</span>
                  <span style={{fontSize:13,fontWeight:700,color:hovered.tradePnL>=0?'#22C759':'#FF3B30',fontFamily:'JetBrains Mono,monospace'}}>{hovered.tradePnL>=0?'+':''}{hovered.tradePnL>=0?'$':'-$'}{Math.abs(hovered.tradePnL).toFixed(2)}</span>
                </div>
                <div style={{display:'flex',justifyContent:'space-between',marginBottom:hovered.drawdown>0?5:0,paddingBottom:hovered.drawdown>0?5:0,borderBottom:hovered.drawdown>0?'1px solid #2A2F3E':'none'}}>
                  <span style={{fontSize:10,color:'#555C70'}}>Cumulé</span>
                  <span style={{fontSize:12,fontWeight:600,color:'#F0F3FF',fontFamily:'JetBrains Mono,monospace'}}>{hovered.cumPnL>=0?'+$':'-$'}{Math.abs(hovered.cumPnL).toFixed(2)}</span>
                </div>
                {hovered.drawdown>0&&<div style={{display:'flex',justifyContent:'space-between'}}>
                  <span style={{fontSize:10,color:'#555C70'}}>Drawdown</span>
                  <span style={{fontSize:11,fontWeight:600,color:'#FF9500',fontFamily:'JetBrains Mono,monospace'}}>-{hovered.drawdown.toFixed(1)}%</span>
                </div>}
              </div>
            )
          })()}
    ...
    <div style={{padding:'28px 28px 60px',maxWidth:1200,margin:'0 auto'}}>
      ...
      <div style={{...card(),marginBottom:16}}>
        ...
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'baseline',marginBottom:12}}>
          <div>
            <div style={{fontSize:11,color:'#555C70',textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:3}}>Courbe P&L cumulée</div>
            {!loading&&<div style={{fontSize:24,fontWeight:700,color:s.totalPnL>=0?'#22C759':'#FF3B30',fontFamily:'JetBrains Mono, monospace',lineHeight:1}}>{fmtK(s.totalPnL)}</div>}
          </div>
          <div style={{display:'flex',gap:16,fontSize:11,fontFamily:'JetBrains Mono,monospace'}}>
            {!loading&&<>
              <span style={{color:'#555C70'}}>WR <span style={{color:s.winRate>=0.5?'#22C759':'#FF3B30',fontWeight:700}}>{(s.winRate*100).toFixed(0)}%</span></span>
              <span style={{color:'#555C70'}}>DD <span style={{color:'#FF9500',fontWeight:700}}>{fmtK(s.maxDD)}</span></span>
            </>}
          </div>
        </div>
