...
function EmotionCurve({ moods }: { moods: MoodEntry[] }) {
  ...
  useEffect(() => {
    ...
    const zones = [
      { y1:5, y2:3.5, color:'rgba(34,199,89,0.04)', label:'Zone optimale' },
      { y1:3.5, y2:2.5, color:'rgba(255,149,0,0.03)', label:'Zone neutre' },
      { y1:2.5, y2:0.5, color:'rgba(255,59,48,0.04)', label:'Zone à risque' },
    ]
    zones.forEach(z => {
    ...
    ctx.fillStyle = '#22C759'; ctx.fillText('😎 5', PAD.l - 4, toY(5) + 4)
    ctx.fillStyle = '#8F94A3'; ctx.fillText('😐 3', PAD.l - 4, toY(3) + 4)
    ctx.fillStyle = '#FF3B30'; ctx.fillText('😰 1', PAD.l - 4, toY(1) + 4)

    // Moving average fill
    const maG = ctx.createLinearGradient(0, PAD.t, 0, PAD.t + cH)
    maG.addColorStop(0, 'rgba(191,90,242,0.15)'); maG.addColorStop(1, 'rgba(191,90,242,0.00)')
    ctx.beginPath()
    ...
    // Moving average line
    ctx.beginPath(); ctx.strokeStyle = 'rgba(191,90,242,0.5)'; ctx.lineWidth = 1.5; ctx.setLineDash([4, 3])
    ma.forEach((v, i) => i === 0 ? ctx.moveTo(toX(i), toY(v)) : ctx.lineTo(toX(i), toY(v)))
    ...
    for (let i = 1; i < points.length; i++) {
      ...
      ctx.strokeStyle = points[i].score >= 3.5 ? '#22C759' : points[i].score >= 2.5 ? '#FF9500' : '#FF3B30'
    ...
    <div ref={wrapRef} style={{ background:'#161B22', border:'1px solid #2A2F3E', borderRadius:12, padding:'14px 16px', marginBottom:16 }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          ...
          <div>
            <div style={{ fontSize:13, fontWeight:700, color:'#F0F3FF' }}>Courbe émotionnelle</div>
            <div style={{ fontSize:10, color:'#555C70' }}>{points.length} entrées · moy. {avgScore.toFixed(1)}/5</div>
          </div>
        </div>
        <div style={{ display:'flex', gap:4 }}>
          {(['7j','1M','3M','all'] as const).map(p => (
            <button key={p} onClick={() => setPeriod(p)} style={{
              ...
              border:`1px solid ${period === p ? '#BF5AF2' : '#2A2F3E'}`,
              background: period === p ? 'rgba(191,90,242,0.15)' : 'transparent',
              color: period === p ? '#BF5AF2' : '#555C70',
            }}>{p === 'all' ? 'Tout' : p}</button>
          ))}
        </div>
      </div>
      ...
      {/* Hover info */}
      {hoveredPt && (
        <div style={{ display:'flex', alignItems:'center', gap:10, padding:'6px 10px', background:'rgba(191,90,242,0.06)', border:'1px solid rgba(191,90,242,0.15)', borderRadius:8, marginBottom:8 }}>
          ...
          <div style={{ flex:1 }}>
            <div style={{ display:'flex', alignItems:'center', gap:8 }}>
              ...
              <span style={{ fontSize:10, color:'#555C70', fontFamily:'JetBrains Mono,monospace' }}>Score: {hoveredPt.score}/5</span>
              <span style={{ fontSize:10, color:'#555C70' }}>·</span>
              <span style={{ fontSize:10, color:'#8F94A3' }}>Intensité: {hoveredPt.intensity}/10</span>
            </div>
            <div style={{ fontSize:10, color:'#555C70', fontFamily:'JetBrains Mono,monospace' }}>
              {hoveredPt.date.toLocaleDateString('fr-FR', { weekday:'short', day:'2-digit', month:'short', hour:'2-digit', minute:'2-digit' })}
            </div>
          </div>
          {hoveredPt.notes && <div style={{ fontSize:10, color:'#8F94A3', maxWidth:200, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>"{hoveredPt.notes}"</div>}
        </div>
      )}
      ...
      <div style={{ display:'flex', gap:14, marginTop:8, justifyContent:'center' }}>
        {[
          { color:'#22C759', label:'Zone optimale (3.5-5)' },
          { color:'#FF9500', label:'Zone neutre (2.5-3.5)' },
          { color:'#FF3B30', label:'Zone à risque (1-2.5)' },
          { color:'rgba(191,90,242,0.5)', label:'Moyenne mobile', dash:true },
